...
import { getThemeMode, type ThemeMode } from '@/lib/settings';
...
export function ThemeProvider({ children }: ThemeProviderProps) {
  ...
  if (!loaded) {
    const bgColor = themeColors[activeColorScheme === 'dark' ? 'dark' : 'light'].background;
    return (
      <View style={[themeVars, { flex: 1, backgroundColor: bgColor }]} className={`${activeColorScheme} flex-1`} />
    );
  }
